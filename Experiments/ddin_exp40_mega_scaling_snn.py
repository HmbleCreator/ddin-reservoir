...
print("="*65)
print("DDIN Exp 40 -- MEGA-SCALING (2000 ROOTS)")
print("Phase 10: Language-Scale Attractor Dynamics")
print("="*65)

# ─────────────────────────────────────────────────────────────────
# 1. TRANSLITERATOR (Devanagari -> SLP1)
# ─────────────────────────────────────────────────────────────────
...
def process_corpus(model, dataset):
    ...
    for i, item in enumerate(dataset):
        ...
        # Use primary inferred axis as label
...
corpus = raw_data['data'][:2000] # Mega-Run 2000

model = MegaSNN().to(device)
states, labels = process_corpus(model, corpus)

# Filter out unlabelled roots for ARI calculation
mask = labels != -1
valid_states = states[mask]
valid_labels = labels[mask]

print(f"\nClustering {len(valid_states)} identified roots...")
...
print(f"\nMEGA-RUN RESULTS (2000 ROOTS):")
print(f"  ARI (Semantic Alignment): {ari:.4f}")
print(f"  Mean Activity: {states.mean():.2f} spikes/root")
print(f"  Identified Classes: {len(valid_states)} / 2000")
